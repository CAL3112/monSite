<h1 class="page">Projets</h1>
<p class="info-responsive">La plupart de ces projets ont été conçu pour un affichage sur ordinateur, l'affichage sur tablette ou mobile peut être non adapté.</p>
<section class="section section-projet">

<div class="div-projet">
    <div class="div-text-projet">
        <p>
            <b>Réalisation d'un site vitrine pour une coach en développement personnel</b><br><br>
            <i>Ce projet de groupe consistait à créer un site vitrine statique pour une activité de coaching en développement personnel, il m'a permis de mettre en application les compétences aquise en HTML et CSS lors de ma remise à niveau.<br><br>(05/2021)<br><br></i>
        </p>

    </div>
    <div class="div-lien-projet"><a class="lien-projet" href="projets/SiteCatherine/index.html" target="_blank">Voir le site</a></div>
</div>

<div class="div-projet">
    <div class="div-text-projet">
        <p>
            <b>Réalisation d'une maquette pour un club de handball en utilisant le logiciel Figma</b><br><br>
            <i>L'objectif de ce projet de graphisme était de créer une maquette de site internet en utilisant le logiciel Figma. J'ai pu, lors de ce projet, perfectionner ma connaissance du logiciel Figma ainsi que renforcer les acquis en compétences de graphismes.<br><br>(09/2021)<br><br></i>
        </p>
    </div>
    <div class="div-colonne">
    <div class="div-lien-projet2"><a class="lien-projet" href="https://www.figma.com/proto/YnXJAQrwOhJui65cLOUIIO/PRINCIPALE?node-id=313%3A2&scaling=min-zoom&page-id=0%3A1&starting-point-node-id=407%3A8552" target="_blank">Maquette PC</a>
    </div>
    <div class="div-lien-projet2"><a class="lien-projet" href="https://www.figma.com/proto/YnXJAQrwOhJui65cLOUIIO/PRINCIPALE?node-id=385%3A332&scaling=scale-down&page-id=357%3A200&starting-point-node-id=412%3A2028" target="_blank">Maquette Mobile</a>
    </div>
    </div>
</div>

<div class="div-projet">
    <div class="div-text-projet">
        <p>
            <b>Réalisation d'un mini jeu en Java Script</b><br><br>
            <i>Mini jeu de rapidité et de précision où l'on doit cliquer sur des cibles qui apparaîssent de manière aléatoire. L'objectif de ce mini projet était de manipuler les fonctions de bases de JavaScript<br><br>(09/2021)<br><br></i>
        </p>
    </div>
    <div class="div-lien-projet"><a class="lien-projet" href="projets/JeuDesCibles/index.html" target="_blank">Jeu des cibles</a></div>
</div>

<div class="div-projet">
    <div class="div-text-projet">
        <p>
            <b>Gestion de données et administration</b><br><br>
            <i><br><br>(10/2021)<br><br></i>
        </p>
    </div>
        <div class="div-lien-projet"><a class="lien-projet" href="projets/GestionDesCochon/index.php" target="_blank">Gestion des cochons</a></div>
</div>


<h1 class="projets-en-cours">D'autres projets sont en cours <i class="fas fa-sync-alt loading"></i></i></h1>

</section>
